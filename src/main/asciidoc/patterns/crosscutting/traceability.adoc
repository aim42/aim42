...
When discussing problems, some stakeholders will question or doubt your findings (see pattern <<Expect-Denial>>). Keeping thorough references to the origins or original sources of major
findings keep eventual critics in check.
...
back to an original source (i.e. an interview protocol, a particular file/version of source code). These stakeholders immediately suspected _all_ our findings, not only the one or two in question. 
